...
public class ProfessorDAO {
	...
	public List<Professor> getTodosProfessores() {
		...
		try (Connection conn = DatabaseUtil.getConexao();
			...
			while (rs.next()) {
				Professor professor = new Professor(rs.getInt("id"), rs.getString("nome"), rs.getString("email"),
						rs.getString("lattes"), rs.getString("login"), rs.getString("senha"), rs.getString("areaAtuacao"));
	...
	public List<Professor> getTodosProfessoresSemId() {
		String sql = "SELECT id, nome, email, lattes, login, senha, areaAtuacao FROM professor";
		List<Professor> professores = new ArrayList<>();
		try (Connection conn = DatabaseUtil.getConexao();
				PreparedStatement stmt = conn.prepareStatement(sql);
				ResultSet rs = stmt.executeQuery()) {
	...
	public List<Professor> getTodosProfessoresSemIdSenhaLogin() {
		...
		try (Connection conn = DatabaseUtil.getConexao();
			...
			while (rs.next()) {
				Professor professor = new Professor(rs.getString("matricula"), rs.getString("nome"), rs.getString("email"),
						rs.getString("lattes"));
				professores.add(professor);
			}
		} catch (SQLException e) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) {
					professor = new Professor(rs.getInt("id"), rs.getString("nome"),
							rs.getString("email"), rs.getString("lattes"), rs.getString("login"),
							rs.getString("senha"), rs.getString("areaAtuacao"));
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
